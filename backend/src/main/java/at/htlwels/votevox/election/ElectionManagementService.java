...
public class ElectionManagementService {
    ...
    public void delete(UUID id) {
        ...
        // tokens has no election FK, so the DB cascade only removes the
        // election_tokens ledger - clean up the token rows explicitly to avoid
        // orphans lingering after the election is gone.
        List<UUID> tokenIds = electionTokenRepository.findAllByElectionId(id).stream()
    ...
    private ElectionResponse toResponse(Election e) {
        return new ElectionResponse(
                ...
                electionClassRepository.sumStudentCountByElectionId(e.getId()),

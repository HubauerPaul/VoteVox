...
import { ClassesTab } from './election-tabs/ClassesTab';
import { TokensTab } from './election-tabs/TokensTab';
import { ResultsTab } from './election-tabs/ResultsTab';

type TabKey = 'overview' | 'candidates' | 'classes' | 'tokens' | 'results';
...
export function ElectionDetailPage(): JSX.Element {
  ...
  // Only block the whole page on the FIRST load. Background refreshes (e.g.
  // after generating tokens) must not unmount the tabs - doing so would throw
  // away in-tab state like the freshly generated, one-time token plaintexts.
  if (loading && !election) {
  ...
  return (
    <>
      ...
      {currentTab === 'overview' && (
        <Card title="Overview">
          <dl className="kvp-list">
            ...
            <dt>Start date</dt>
            <dd>{formatDateTime(election.startTime, 'date')}</dd>
            <dt>End date</dt>
            <dd>{formatDateTime(election.endTime, 'date')}</dd>
            <dt>Description</dt>
      ...
      {currentTab === 'classes' && (
        <ClassesTab electionId={election.id} status={election.status} onChanged={loadElection} />
      )}

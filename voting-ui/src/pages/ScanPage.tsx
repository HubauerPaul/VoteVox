...
/**
 * Stops and clears the scanner without ever throwing.
 *
 * html5-qrcode's `stop()` throws *synchronously* ("Cannot stop, scanner is not
 * running or paused") when the scanner isn't in a running/paused state. That
 * happens routinely in React 18 StrictMode, where the effect cleanup fires
 * before `start()` has resolved. We guard on the live scanner state and swallow
 * any residual error so it can never bubble up to the router error boundary.
 */
async function safeStopScanner(scanner: Html5Qrcode | null): Promise<void> {
  ...
  try {
    ...
    // Scanner was never fully started (or already stopped) — nothing to do.
  }
}
    ...
    async function startScanner(): Promise<void> {
      try {
        ...
        if (cancelled) {
          ...
          // (e.g. StrictMode's double-invoke). Stop the now-running scanner.
          void safeStopScanner(scanner);
          return;
        }
